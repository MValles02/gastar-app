...
import MultiSelect from '../../../shared/components/ui/MultiSelect.js';
...
import SpendingByCategory from '../../dashboard/components/SpendingByCategory.js';
...
import type { ByCategoryData, CategoryReportItem } from '../services/reports.js';
...
function computeNetByCategory(categoryData: ByCategoryData | null): CategoryReportItem[] {
...
export default function SpendByCategory(): JSX.Element {
  ...
  const update = (key: keyof LocalFilters, value: string | string[]) =>
    setLocalFilters((prev) => ({ ...prev, [key]: value }));

  const load = useCallback(async (filters: LocalFilters) => {
    setLoading(true);
    setError('');
    try {
      const params = {
  ...
  const apply = () => void load(localFilters);
