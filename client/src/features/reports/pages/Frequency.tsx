...
import type { FrequencyItem } from '../services/reports.js';
...
export default function Frequency(): JSX.Element {
  ...
  const [data, setData] = useState<FrequencyItem[] | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [appliedFilters, setAppliedFilters] = useState<{ from: string; to: string } | null>(null);
      ...
      <div className="space-y-6">
        ...
        {error ? (
          <PageErrorState
            ...
            message={error}
            onAction={() => appliedFilters && void load(appliedFilters.from, appliedFilters.to)}
          />
        ) : null}
        ...
        {!loading && !error && data ? (
          data.length === 0 ? (
            ...
            <div className="list-surface divide-y divide-border-default/40">
              {data.map((item) => {
                const Icon = getCategoryIcon(item.categoryIcon ?? null) ?? Tag;
