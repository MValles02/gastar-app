...
import { getCategoryIcon } from '../../../shared/components/ui/IconPicker.jsx';
import { formatCurrency, formatDate, getAmountTone } from '../../../shared/utils/formatters.js';
import { transactionShape } from '../../../shared/utils/propTypes.js';
import { TYPE_CONFIG, amountPrefix } from '../../../shared/constants/transactionTypes.js';
        ...
        <section key={dateKey} className="section-block">
          ...
          <div className="list-surface">
            {items.map((tx) => {
              ...
              const { Icon: TypeIcon, tone: typeTone } =
                TYPE_CONFIG[tx.type] ?? TYPE_CONFIG.expense;
                ...
                <div key={tx.id} className="list-row flex-col items-start gap-1.5">
                  ...
                  <div className="flex items-center gap-3 w-full">
                    ...
                    >
                      ...
                      {formatCurrency(Number.parseFloat(tx.amount), tx.account?.currency || 'ARS')}
                  ...
                  <div className="flex items-center gap-2 w-full pl-7">
                    <p className="flex items-center gap-1 truncate text-xs text-app-muted flex-1">
                      ...
                      {tx.category?.name ? (
                        <span className="font-medium text-app-soft shrink-0">
                          {tx.category.name}
                        </span>
                      ) : null}
                      ...
                      {tx.type === 'transfer' && tx.transferToAccount ? (
                        <>
                          <ArrowRight className="inline h-3 w-3 shrink-0" />{' '}
                      ...
                      {tx.account?.currency && tx.account.currency !== 'ARS' && tx.cotizacion ? (
                        <span className="shrink-0">
                          {' '}
                          · @{formatCurrency(Number.parseFloat(tx.cotizacion))}
                        </span>
                      ) : null}
                    </p>
